 ...
 * 
 * - 작성된 구문에 마즌ㄴ 행, 열 데이터가 조회됨
 *   -> RRESULT SET은 0행 이상이 포함될 수 있다
 *   -> 조건에 맞는 횅이 없을 수도 있기 때문에
 *  */
...
-- EMPLYEE 테이블에서
-- 모든 햄의 사번(EMP_ID), 이름(EMP_NAME), 입사일(HIRE_DATE) 조회

SELECT EMP_ID, EMP_NAME, SALARY FROM EMPLOYEE;
SELECT EMP_NAME, HIRE_DATE FROM EMPLOYEE;

-- EMPLOYEE 테이블의 모든 행, 모든 컬럼 조회
-- asterisk (*) : 모든, 포함하다의 뜻을 지닌 기호
...
-- 컬럼 값 : 행과 열이 교차되는 테이블의 한 ㅋ ㅏㄴ에 작성된 값
...
-- CEIL(숫자) : 올림 처리
-- 현재 시간 - 생일
SELECT CEIL ( (SYSDATE - TO_DATE('1997-07-1', 'YYYY-MM-DD')) / 365 )
FROM DUAL;
...
SELECT CEIL ( (SYSDATE - TO_DATE('1997-07-1', 'YYYY-MM-DD')) / 365 ) AS 나이
FROM DUAL;


-- EMPLOYEE 테이블에서
-- 사번, 사원 이름, 연봉(급여*12) 조회
-- 단, 컬럼명은 위에 작성된 그대로 조회

SELECT EMP_ID AS 사번, EMP_NAME AS "사원 이름", SALARY * 12 AS 연봉, SALARY 급여
FROM EMPLOYEE;
...
SELECT  EMP_NAME , SAL_LEVEL , '원', SYSDATE 
FROM EMPLOYEE;
...
SELECT EMP_ID , EMP_NAME , SALARY 
FROM EMPLOYEE
WHERE NOT SALARY >= 3000000 OR NOT SALARY <= 5000000;
...
/*2*/SELECT EMP_NAME, SALARY
/*1*/FROM EMPLOYEE
/*3*/ORDER BY SALARY DESC;  -- ASC(descending) : 내림차순
...
FROM EMPLOYEE
WHERE DEPT_CODE IN ('D5', 'D6', 'D9')
ORDER BY DEPT_CODE ASC; -- ASC 생략 가능 (기본 값이다!!)
...
-- 내림차순 + NULLS LAST (NULL인 경우 제일 아래)
SELECT EMP_NAME, PHONE FROM EMPLOYEE
