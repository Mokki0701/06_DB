...
-- 부서명이 '해외영업2부'인 사원의
-- 사번, 이름, 부서명(DEPT_TITLE)을 사번 오름차순으로 조회
...
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_ID = DEPT_CODE AND DEPT_TITLE = '해외영업2부'
ORDER BY EMP_ID;
...
-- 오라클 구문
SELECT EMP.EMP_NAME 사원명, MGR.EMP_NAME 사수
...
AND SUBSTR(EMP_NO,8,1) = 2 AND SUBSTR(EMP_NAME,1,1) = '전' ;
...
SELECT EMP_NAME , JOB_NAME , SALARY ,
			CASE 
				...
				WHEN BONUS IS NOT NULL THEN SALARY*12*(1+BONUS)
			END 연봉
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
